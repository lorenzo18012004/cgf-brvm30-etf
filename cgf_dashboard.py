...
if _page == "backtest":
    ...
    # ── Vue d'ensemble ────────────────────────────────────────────────────────
        ...
        with col1:
            ...
            c1b.metric("Titres moy.", str(bm.get("n_titres_avg", "—")))
            c2b.metric("Titres min.", str(bm.get("n_titres_min", "—")))
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        def _live_fragment():
            ...
            if len(_etf_cl) >= 2 and len(_idx_cl) >= 2:
                ...
                if len(_common) >= 1:
                    ...
                    _te = float(_active.std() * _np.sqrt(252) * 100) if len(_common) >= 2 else float(abs(_active.iloc[0]) * _np.sqrt(252) * 100)
            ...
            _non_repr = _n_seances is not None and _n_seances < 30
                ...
                if True:
                    ...
                    if _idx_open and any(v is not None for v in idx_pts):
                        ...
                        for _ai in range(1, len(_snaps_c)):
                            ...
                            # = Σ(w_brvm30_i × ret_i) — identité exacte avec Σ(gap_contrib)
                            ...
                            # Pour l'attribution : estimé (synchrone). Pour l'affichage : officiel en secondaire
                        ...
                        if _attr_data:
                            ...
                            st.caption(
                                ...
                                "BRVM30* = valeur estimee depuis nos propres prix (synchrone avec l'iNAV). "
                                "L'indice officiel Sika ne se met a jour que toutes les 15-30 min."
                            )
                            _items_html = ""
                            for _i, _d in enumerate(_attr_data):
                                ...
                                # BRVM30 officiel pour info (peut être figé)
                                _id_off = _d.get('idx_d_off')
                                _off_diff = _id_off is not None and _id is not None and abs((_id_off or 0) - _id) > 0.002
                                _brv_label = 'BRVM30*' if _d.get('contribs') else 'BRVM30'
                                _off_note = (f' <span style="color:#9ca3af;font-size:10px">(officiel {_id_off:+.3f}%)</span>' if _off_diff else '')
                                ...
                                _summary = (
                                    ...
                                    f'<span style="color:#9ca3af;font-size:11px">{_brv_label}</span>'
    ...
    # ── Rebalancements ────────────────────────────────────────────────────────
        ...
        if not rd_data or not rd_data.get("rebalancings"):
            ...
            with st.expander("✏️ Corriger une composition existante", expanded=False):
                ...
                _TICKER_NAMES = {
                    'ABJC': 'Servair Abidjan', 'BICB': 'BIC Bénin', 'BICC': 'BICICI',
                    ...
                    'BOAS': 'Bank of Africa Sénégal', 'CABC': 'Compagnie Africaine de Banque CI',
                    'CBIBF': 'Coris Bank International Burkina', 'CFAC': 'CFAC CI',
                    'CIEC': 'CIE (Électricité CI)', 'ECOC': 'Ecobank CI',
                    'ETIT': 'Etitre', 'FTSC': 'Filtisac CI', 'LNBB': 'LONAB Bénin',
                    ...
                    'STBC': 'Société Générale Banques Burkina', 'SVOC': 'SIVOA CI',
                    'TTLC': 'TotalEnergies CI', 'TTLS': 'TotalEnergies Sénégal',
                    'UNLC': 'Unilever CI', 'UNXC': 'Unilever CI (UNXC)',
                }
    ...
    # ── Analyse approfondie ───────────────────────────────────────────────────
        ...
        if rd_data and rd_data.get("rebalancings"):
            last = rd_data["rebalancings"][-1]
            st.markdown("---")
            _section(f"Dernier rebalancement — {last['date_label']} — {last['basket_n']} titres")
            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
            col_m1.metric("Turnover",   f"{last['turnover']*100:.1f}%")
            col_m2.metric("Coût",       f"{last['cost_bps']:.0f} bps")
            col_m3.metric("Exclusions", f"{last['excl_n']} titres ({last['excl_w']*100:.1f}%)")
            col_m4.metric("Couverture", f"{last.get('coverage', 0)*100:.1f}%")

            tb_basket, tb_excl = st.tabs(["Panier & Trades", "Exclusions"])
            with tb_basket:
                if last.get("basket"):
                    ...
                    def _liq_ratio(row):
                        adv = row.get("adv_mfcfa", 0)
                        tr  = abs(row.get("trade_mfcfa", 0))
                        if adv and adv > 0 and tr > 0:
                            r = tr / adv
                            return f"{r:.2f}x"
                        return "—"
                    df_b["Liq."] = df_b.apply(_liq_ratio, axis=1)
                    def _color_trade_a(val):
                        if isinstance(val, (int, float)):
                            c = "#2d7a4f" if val > 0 else "#c0392b" if val < 0 else ""
                            return f"color: {c}; font-weight: 500"
                        return ""
                    ...
                    tc3.metric("Net",    f"{total_buy + total_sell:,.0f} MFCFA")
            with tb_excl:
                if last.get("excluded"):
                    ...
                    st.dataframe(df_e.rename(columns={
                        "ticker": "Ticker", "secteur": "Secteur",
                        "w_brvm30": "BRVM30 %", "adv_mfcfa": "ADV (MFCFA)",
                        "adv_req": "ADV requis", "ratio_adv": "Ratio ADV",
                        "raison": "Raison", "trade_mfcfa": "Trade (vente)", "days_exec": "J. exec.",
