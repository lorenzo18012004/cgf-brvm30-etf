...
class IntradayScraper(BaseScript):
    def __init__(self):
        super().__init__()
    ...
    def run(self, force = False):
        ...
        if not force and not self._is_market_open():
            self._log(f"Hors heures de marche ({now_utc.strftime('%H:%M')} UTC) — rien a faire")
            return None
        ...
        try:
            ...
            self._log(f"ERREUR calcul VL : {e}")
            return None

        data = self._load_intraday()

        if data.get('date') != today_str:
            data = {
                'date':      today_str,
                'snapshots': [],
                'open_nav':  nav_result['nav_indice'],
            }
        ...
        try:
            ...
            if _rebal_date:
                try:
                    ...
                    with open(os.path.join(self.data_dir, 'richbourse_history.json'), encoding='utf-8') as _f3:
                        _rh = json.load(_f3)
            ...
            self._log(f"Contributions calc error: {_e}")
        ...
        for _i, _s in enumerate(_snaps):
            ...
            if _prev is not None and _next is not None:
                ...
                if _t1 > _t0:
                    ...
                    self._log(f"Interpolation brvm30_official {_s['time']}: {_s['brvm30_official']} (entre {_snaps[_prev]['time']} et {_snaps[_next]['time']})")

        self._save_intraday(data)

        hist_path = os.path.join(self.data_dir, 'nav_intraday_history.json')
        try:
        ...
        try:
            import subprocess
            push_bat = os.path.join(self.data_dir, 'push_data.bat')
            if os.path.exists(push_bat):
                subprocess.Popen([push_bat], creationflags=0x08000000)
        except Exception:
            pass
        ...
        self._log(f"iNAV={nav_result['nav_indice']:.4f} VL={vl_display:,.0f} FCFA jour={change_day:+.3f}%{launch_str}")
...
if __name__ == '__main__':
    ...
    result = IntradayScraper().run(force=args.force)
