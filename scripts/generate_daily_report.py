...
class ReportGenerator(BaseScript):
    ...
    def _chart_base100(self, intra_hist, launch_date, nav_at_launch, par):
        fig, ax = plt.subplots(figsize=(13, 3.5))
        fig.patch.set_facecolor('#ffffff')
        ax.set_facecolor('#f8fafc')
    ...
    def _styles(self):
        ...
        return {
            'title':    ParagraphStyle('rpt_title',   fontName='Helvetica-Bold', fontSize=20, textColor=self.CGF_DARK, spaceAfter=2, leading=24),
            'subtitle': ParagraphStyle('rpt_sub',     fontName='Helvetica',      fontSize=10, textColor=self.CGF_GRAY, spaceAfter=2),
            'h2':       ParagraphStyle('rpt_h2',      fontName='Helvetica-Bold', fontSize=12, textColor=self.CGF_DARK, spaceBefore=12, spaceAfter=5),
            'h3':       ParagraphStyle('rpt_h3',      fontName='Helvetica-Bold', fontSize=10, textColor=self.CGF_BLUE, spaceBefore=8,  spaceAfter=4),
            'body':     ParagraphStyle('rpt_body',    fontName='Helvetica',      fontSize=9,  textColor=self.BLACK, leading=13),
            'note':     ParagraphStyle('rpt_note',    fontName='Helvetica-Oblique', fontSize=7.5, textColor=self.CGF_GRAY, leading=10),
            'rdate':    ParagraphStyle('rpt_date',    fontName='Helvetica',      fontSize=10, textColor=self.CGF_GRAY, alignment=TA_RIGHT, leading=14),
            'mv_big':   ParagraphStyle('rpt_mv_big',  fontName='Helvetica-Bold', fontSize=15, textColor=self.CGF_DARK, alignment=TA_CENTER, leading=18),
            'mv_pos':   ParagraphStyle('rpt_mv_pos',  fontName='Helvetica-Bold', fontSize=15, textColor=self.CGF_GREEN, alignment=TA_CENTER, leading=18),
            'mv_neg':   ParagraphStyle('rpt_mv_neg',  fontName='Helvetica-Bold', fontSize=15, textColor=self.CGF_RED,   alignment=TA_CENTER, leading=18),
            'mv_neu':   ParagraphStyle('rpt_mv_neu',  fontName='Helvetica-Bold', fontSize=15, textColor=self.CGF_GRAY,  alignment=TA_CENTER, leading=18),
            'mv_label': ParagraphStyle('rpt_mv_lbl',  fontName='Helvetica',      fontSize=7.5, textColor=self.CGF_GRAY, alignment=TA_CENTER, leading=9),
        }

    def _val_style(self, S, val):
        if val is None:
            return S['mv_big']
        return S['mv_pos'] if val > 0 else (S['mv_neg'] if val < 0 else S['mv_neu'])

    def _fmt_pct(self, v, decimals=3):
        if v is None:
            return '—'
        sign = '+' if v > 0 else ''
    ...
    def generate(self, report_date: str = None, force: bool = False) -> str:
        ...
        story.append(header)
        story.append(HRFlowable(width=cw, thickness=3, color=self.CGF_BLUE, spaceAfter=8))
        ...
        row1_vals = [
            ...
            _mcell(self._fmt_pct(var_jour), 'Variation du jour', _vl_style(var_jour)),
        ...
        m1.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), self.CGF_LIGHT),
            ('BOX', (0, 0), (-1, -1), 1, self.CGF_BORDER),
            ('LINEAFTER', (0, 0), (2, 0), 0.5, self.CGF_BORDER),
        ...
        row2_vals = [
            _mcell(self._fmt_pct(perf_launch), f'Perf. ETF depuis le {launch_date}', _vl_style(perf_launch)),
            _mcell(self._fmt_pct(perf_idx),    'Perf. BRVM30 même période',           _vl_style(perf_idx)),
        ...
        td_str = self._fmt_pct(td, 4) if td is not None else '— (min. 2 séances)'
        ...
        te_tbl.setStyle(TableStyle([
            ...
            ('FONTSIZE',   (0, 0), (-1, -1), 9),
            ('TEXTCOLOR',  (0, 0), (-1, -1), self.BLACK),
            ('BACKGROUND', (0, 0), (0, -1), self.CGF_LIGHT),
            ('BACKGROUND', (2, 0), (2, -1), self.CGF_LIGHT),
            ('BOX',        (0, 0), (-1, -1), 1, self.CGF_BORDER),
            ('INNERGRID',  (0, 0), (-1, -1), 0.5, self.CGF_BORDER),
        ...
        story.append(HRFlowable(width=cw, thickness=1, color=self.CGF_BORDER, spaceAfter=8))
        ...
        n_days_data = sum(1 for d, pts in ih.items()
                          if pts and pd.Timestamp(d) >= launch_ts)
        if n_days_data >= 1:
            buf2 = self._chart_base100(ih, launch_date, nav_at_launch, par)
        ...
        story.append(HRFlowable(width=cw, thickness=1, color=self.CGF_BORDER, spaceAfter=6))
        story.append(Paragraph(
        ...
        style_cmds = [
            ('BACKGROUND', (0, 0), (-1, 0), self.CGF_DARK),
            ...
            ('BOX',        (0, 0), (-1, -1), 1, self.CGF_BORDER),
            ('LINEBELOW',  (0, 0), (-1, 0),  1, self.CGF_BLUE),
            ('GRID',       (0, 1), (-1, -1), 0.5, self.CGF_BORDER),
        ]

        for i in range(1, len(tbl_data)):
            if i % 2 == 0:
                style_cmds.append(('BACKGROUND', (0, i), (-1, i), self.CGF_ROW))
            v = variations[i - 1]
            if v is not None and v > 0:
                style_cmds.append(('TEXTCOLOR', (3, i), (3, i), self.CGF_GREEN))
                style_cmds.append(('FONTNAME',  (3, i), (3, i), 'Helvetica-Bold'))
            elif v is not None and v < 0:
                style_cmds.append(('TEXTCOLOR', (3, i), (3, i), self.CGF_RED))
                style_cmds.append(('FONTNAME',  (3, i), (3, i), 'Helvetica-Bold'))
        ...
        if basket_var:
            ...
            tb_style = [
                ...
                ('BACKGROUND', (0, 0), (2, 0), self.CGF_GREEN),
                ('BACKGROUND', (3, 0), (5, 0), self.CGF_RED),
                ('TEXTCOLOR',  (0, 0), (5, 0), self.WHITE),
                ('BACKGROUND', (0, 1), (2, 1), self.CGF_GLIGHT),
                ('BACKGROUND', (3, 1), (5, 1), self.CGF_RLIGHT),
                ...
                ('BOTTOMPADDING', (0, 0), (-1, -1), 4),
                ('BOX',   (0, 0), (-1, -1), 1, self.CGF_BORDER),
                ('GRID',  (0, 0), (-1, -1), 0.5, self.CGF_BORDER),
            ]
            for i in range(2, len(tb_data)):
                if i - 2 < len(top5):
                    tb_style.append(('TEXTCOLOR', (2, i), (2, i), self.CGF_GREEN))
                    tb_style.append(('FONTNAME',  (2, i), (2, i), 'Helvetica-Bold'))
                if i - 2 < len(bottom5):
                    tb_style.append(('TEXTCOLOR', (5, i), (5, i), self.CGF_RED))
                    tb_style.append(('FONTNAME',  (5, i), (5, i), 'Helvetica-Bold'))
            tb_tbl.setStyle(TableStyle(tb_style))
            story.append(tb_tbl)

        story.append(Spacer(1, 16))
        story.append(HRFlowable(width=cw, thickness=0.5, color=self.CGF_BORDER, spaceAfter=5))
        story.append(Paragraph(
