...
SIKA_HIST     = os.path.join(BASE_DIR, "sika_history.json")
NAV_LATEST    = os.path.join(BASE_DIR, "nav_latest.json")
DIV_CALENDAR  = os.path.join(BASE_DIR, "dividend_calendar.json")

# Seuils de détection
THRESHOLD_DIV   = 0.04   # >4% de baisse → possible dividende
THRESHOLD_SPLIT = 0.15   # >15% de variation → possible split / événement majeur
CALENDAR_DAYS   = 5      # Alerter les événements du calendrier dans les 5 prochains jours
...
def _get_prev_close(sika_hist, ticker, today_str):
    """Dernier cours de clôture officiel avant aujourd'hui."""
...
def _get_etf_weights(nav_latest):
    """Poids ETF par ticker."""
...
def _check_calendar(today_str):
    """Retourne les événements du calendrier dans les CALENDAR_DAYS prochains jours."""
    cal = _load_json(DIV_CALENDAR) or {}
...
def _send_alert(subject, body_html, gmail_user, gmail_pass):
...
def run():
    ...
    sika_hist  = _load_json(SIKA_HIST) or {}
    nav_latest = _load_json(NAV_LATEST) or {}
    etf_weights = _get_etf_weights(nav_latest)

    # ── 1. Anomalies de prix ───────────────────────────────────────────────────
    live_prices = _scrape_live_prices()
    anomalies = []

    all_tickers = set(etf_weights.keys()) | set(sika_hist.keys())
    for tk in sorted(all_tickers):
        prev_close = _get_prev_close(sika_hist, tk, today_str)
        ...
        severity = None
        if abs(chg) >= THRESHOLD_SPLIT:
            severity = "CRITIQUE"
        elif chg <= -THRESHOLD_DIV:
            severity = "ATTENTION"
    ...
    # ── 2. Calendrier des actions connues ──────────────────────────────────────
    calendar_alerts = _check_calendar(today_str)
    ...
    secrets    = _load_json(os.path.join(BASE_DIR, "secrets.json")) or {}
    ...
    _send_alert(subject, body_html, gmail_user, gmail_pass)
